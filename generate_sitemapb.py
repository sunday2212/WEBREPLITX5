...
def compute_priority(file_path):
    """
    Priority based on depth:
        - root (./index.html etc.) → 1.00
        - one level deep (./folder/) → 0.80
        - deeper → 0.64
    """
...
for html_file in html_files:
    ...
    # Convert file path to URL format with proper encoding
    url_path = html_file[2:] if html_file.startswith('./') else html_file
    # Encode all special characters (spaces, parentheses, ®, etc.) but keep '/'
    ...
    priority.text = compute_priority(html_file)
